...
def test_compile_latex_with_pylatex():
    ...
    sty_pkgs = str(files("data.ResourceFiles.osdag-latex-env.texmf-dist")).replace("\\", "/")
    ...
    \pagestyle{fancy}
    \fancyhead[L]{\textbf{Diagnostics}}
    \fancyhead[R]{\thepage\ of \pageref{LastPage}}
    \definecolor{MyGreen}{rgb}{0.6, 0.8, 0.2}
    '''))

    with doc.create(Section('LaTeX Package Diagnostics')):
        doc.append('This document confirms the successful import and basic usage of all required .sty files.\n')

        with doc.create(Subsection('amsmath')):
            doc.append(NoEscape(r'$\int_0^\infty e^{-x^2} dx = \frac{\sqrt{\pi}}{2}$'))

        with doc.create(Subsection('graphicx')):
            doc.append("Osdag Header Image")
            doc.append(NoEscape(r'\includegraphics[width=0.1\textwidth]{data/ResourceFiles/images/Osdag_header_report.png}'))
    ...
    # Export
    doc.generate_pdf('minimal_latex_package_test', clean=True, compiler=PDFLATEX_PATH, clean_tex=False)
...
def test_compile_latex_with_pylatex_report():
    ...
    \fancyhead[R]{
        \begin{minipage}[t]{0.3\textwidth}
            \includegraphics[width=\linewidth]{data/ResourceFiles/images/Osdag_header_report.png}
        \end{minipage}
    }
    '''))
    ...
    with doc.create(Tabular(r'|>{\raggedright\arraybackslash}p{5cm}||>{\raggedright\arraybackslash}p{10cm}|')) as table:
        ...
        table.add_hline()
        table.add_row(('Date', NoEscape(r'\today')), color='OsdagGreen')
        table.add_hline()
    ...
    # Compile
    doc.generate_pdf('diagnostics_pylatex', compiler=PDFLATEX_PATH, clean=True, clean_tex=False)
